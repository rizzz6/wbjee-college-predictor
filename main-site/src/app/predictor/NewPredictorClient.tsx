...
export default function NewPredictorClient() {
  ...
  const sortedResults = useMemo(() => {
    ...
    return [...filteredResults].sort((a, b) => {
      const valA = a[sortState.column as keyof CollegeData];
      const valB = b[sortState.column as keyof CollegeData];
    ...
    setSortState(prev => {
      if (prev.column === column) {
        if (prev.direction === 'asc') {
          return { column, direction: 'desc', type: column.includes('rank') ? 'number' : 'string' };
        } else if (prev.direction === 'desc') {
          return { column: null, direction: 'none', type: 'string' };
        } else {
          return { column, direction: 'asc', type: column.includes('rank') ? 'number' : 'string' };
        }
      } else {
        return { column, direction: 'asc', type: column.includes('rank') ? 'number' : 'string' };
      }
    ...
    <ErrorBoundary>
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 font-['Inter',sans-serif]">
          ...
          <div className="mb-6">
            ...
            <div className="space-y-2">
              <input
                ...
                min="1"
                max="50000"
                className={`w-full max-w-sm px-4 py-3 border rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 text-lg font-medium focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors ${
                  rankError ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : 'border-slate-300 dark:border-slate-600'
                }`}
              />
       ...
       <section id="results" className="bg-white dark:bg-slate-800 p-8 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700">
             ...
             >
               ...
               <div className="flex items-center justify-between mb-3">
                 <span className={`px-3 py-1 rounded-full text-sm font-semibold ${
                   item.prediction.text === 'Confirm' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-100' :
         ...
         {/* Desktop Table View */}
         <div className="hidden md:block overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-700" role="region" aria-label="College prediction results" aria-live="polite">
                 ...
                 >
                   <td className="px-2 py-4 whitespace-nowrap text-sm text-center" onClick={(e) => e.stopPropagation()}>
                     <button
                       ...
                       className={`text-2xl ${favorites.has(item.id) ? 'text-yellow-500' : 'text-slate-400 hover:text-yellow-500'}`}
                     >
                       ★
                     </button>
                   </td>
   ...
   <div className="space-y-8">
     <div>
       ...
       <ol className="list-decimal list-inside space-y-3 text-slate-700 dark:text-slate-300">
         ...
         <li className="leading-relaxed">Click the star (★) to add colleges to your shortlist for detailed comparison of admission chances</li>

 ...
 * Export as JSON
 */
function exportAsJSON(data: Record<string, unknown>[], templateName: string): { success: boolean; message: string } {
    try {
    ...
    try {
        ...
        const rows = data.map(item => {
            return fields.map(field => {
                ...
                if (field.includes('.')) {
                    const parts = field.split('.')
                    const parent = item[parts[0]] as Record<string, unknown> | undefined

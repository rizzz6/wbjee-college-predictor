...
import { TrendingUp, Users, Eye, EyeOff, Calendar, Award, Building2, MapPin, RefreshCw } from 'lucide-react'
...
export function AnalyticsDashboardWidget() {
    ...
    const fetchAnalytics = async () => {
        ...
        try {
            ...
            const visible = colleges.filter((c: any) => c.isVisible).length
            const hidden = total - visible
            const withDetails = colleges.filter((c: any) => c.detailsReference).length
            const withoutDetails = total - withDetails

            // Recent activity
            const recentlyAdded = colleges.filter((c: any) =>
                new Date(c._createdAt) > thirtyDaysAgo
            ).length
            const recentlyUpdated = colleges.filter((c: any) =>
            ...
            const complete = colleges.filter((c: any) =>
                c.logo && c.description && c.highlights?.length > 0
            ).length
            const completionRate = total > 0 ? Math.round((complete / total) * 100) : 0

            // Average quality score
            const qualityScores = colleges.map((c: any) => {
                let score = 0
                if (c.logo) score += 25

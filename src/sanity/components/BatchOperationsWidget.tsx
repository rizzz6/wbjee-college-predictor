...
export function BatchOperationsWidget() {
    ...
    const fetchColleges = async () => {
        setLoading(true)
        try {
            const data = await client.fetch(`*[_type == "college"] | order(name asc) { _id, name, location, type, isVisible }`)
            setColleges(data)
        } catch (error) {
        ...
        >
            <Stack space={4}>
                ...
                <Flex gap={2}>
                    ...
                    <Select value={filterType} onChange={(e) => setFilterType(e.currentTarget.value as any)} fontSize={1}>
                    ...
                    {filteredColleges.length > 0 ? (
                        <Stack space={1}>
                            {filteredColleges.map(college => (
                                ...
                                >
                                    <Checkbox checked={selected.has(college._id)} onChange={() => toggleSelect(college._id)} />
                                    <Stack space={0} flex={1}>
                                        <Text size={1}>{college.name}</Text>
                                        <Text size={0} muted>{college.location || 'No location'} • {college.type || 'No type'}</Text>
                                    </Stack>

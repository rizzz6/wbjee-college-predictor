...
async function verifyUpstash(): Promise<ValidationResult> {
    ...
    try {
        ...
        const seen = new Set<string>();
        masterData.forEach((record: { institute: string; branch: string; year: number; category: string; quota: string; seat_type: string; round: string }) => {
            const key = JSON.stringify({
                institute: record.institute,
